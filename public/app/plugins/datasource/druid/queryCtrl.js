...
function (angular, _) {
  ...
  module.controller('DruidTargetCtrl', function($scope, $timeout, $sce, templateSrv, $q) { /*, uiSegmentSrv */
    ...
    var validateLimit = function (target, errs) {
      if (!target.limit) {
    ...
    var validateGroupByQuery = function(target, errs) {
    ...
    var validateTopNQuery = function(target, errs) {
      if (!validateLimit(target, errs)) {
        return false;
      }
      if (!target.metric) {
    ...
    var validateSelectorFilter = function(target) {
    ...
    var validateRegexFilter = function(target) {
      if (!target.currentFilter.dimension) {
    ...
    var validateCountAggregator = function(target) {
    ...
    var validateSimpleAggregator = function(type, target) {
      if (!target.currentAggregator.name) {
    ...
    var validateApproxHistogramFoldAggregator = function(target) {
    ...
    var validateSimplePostAggregator = function(type, target) {
    ...
    var validateQuantilePostAggregator = function (target) {
    ...
    var validateArithmeticPostAggregator = function(target) {
      if (!target.currentPostAggregator.name) {
    ...
    var postAggregatorValidators = {
      "arithmetic": validateArithmeticPostAggregator,
      "quantile": validateQuantilePostAggregator
    };

    var arithmeticPostAggregatorFns = {'+': null, '-': null, '*': null, '/': null};
    var defaultQueryType = "timeseries";
    var defaultFilterType = "selector";
    var defaultAggregatorType = "count";
    var defaultPostAggregator = {type: 'arithmetic', 'fn': '+'};
    var customGranularities = ['minute', 'fifteen_minute', 'thirty_minute', 'hour', 'day', 'all'];
    var defaultCustomGranularity = 'minute';
    var defaultLimit = 5;
